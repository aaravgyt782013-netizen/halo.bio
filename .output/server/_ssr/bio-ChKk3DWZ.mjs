//#region node_modules/.nitro/vite/services/ssr/assets/bio-ChKk3DWZ.js
...
*/
function syncToServer(currentUserId) {
	if (typeof window === "undefined" || typeof fetch === "undefined") return;
...
*/
async function uploadMedia(_userId, file, folder) {
	if (file.type.startsWith("audio/")) {
		if (file.size > 3670016) throw new Error("Audio file exceeds 3.5MB. Please use an MP3 URL or compress your track.");
	}
	if (file.type.startsWith("video/")) {
		if (file.size > 5242880) throw new Error("Video file exceeds 5MB. Please use a direct video URL or smaller clip.");
	}
	if (file.type.startsWith("image/") && typeof window !== "undefined") try {
...
async function fetchProfileByUsername(username) {
	try {
		...
		if (typeof window !== "undefined" && typeof fetch !== "undefined") try {
			const res = await fetch(`/api/profile/${encodeURIComponent(cleanUser)}`);
			if (res.ok) {
				const data = await res.json();
...
//#endregion
export { fetchProfileByUsername as a, ensureProtocol as i, auth as n, normalizeUsername as o, db as r, uploadMedia as s, USERNAME_RE as t };
