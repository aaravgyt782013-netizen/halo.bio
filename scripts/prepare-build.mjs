...
dashboard = dashboard.replace(
  ...
  `const applyTheme = (preset: (typeof PRESET_THEMES)[number]) => {
    ...
    patch({ profile_layout: presentation, accent_color: preset.accent, card_opacity: preset.opacity, card_blur: preset.blur, card_radius: preset.radius } as any);
...
// Allow larger background media uploads. The UI/server can reject oversized files elsewhere,
// so keep the limit explicit and consistent at 10 MB.
dashboard = dashboard.replace(/(MAX_(?:VIDEO|BACKGROUND)_SIZE(?:_BYTES)?\s*=\s*)[^;\n]+/g, '$1 10 * 1024 * 1024');
dashboard = dashboard.replace(/(max(?:Size|Bytes)\s*[:=]\s*)(?:1024\s*\*\s*1024|1\s*\*\s*1024\s*\*\s*1024)/gi, '$1 10 * 1024 * 1024');
// Common inline 1 MB checks used by the existing media picker.
dashboard = dashboard.replace(/file\.size\s*>\s*1024\s*\*\s*1024/g, 'file.size > 10 * 1024 * 1024');
dashboard = dashboard.replace(/file\.size\s*>\s*1\s*\*\s*1024\s*\*\s*1024/g, 'file.size > 10 * 1024 * 1024');
dashboard = dashboard.replace(/1\s*MB/gi, '10 MB');
