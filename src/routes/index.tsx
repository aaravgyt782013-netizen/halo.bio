    ...
    <div className="relative min-h-screen overflow-hidden">
      ...
      <header className="mx-auto flex max-w-6xl items-center justify-between px-5 py-6">
        <span className="font-display text-lg font-bold tracking-tight">
          <span className="spider-brand">Spider</span> Website
        </span>
        <nav className="flex items-center gap-2">
          <Link to="/auth" className="btn-ghost">Log in</Link>
          <Link to="/auth" search={{ mode: "signup" }} className="btn-primary">Get started</Link>
        </nav>
      </header>
      ...
      <main className="mx-auto max-w-6xl px-5 pb-24">
        <section className="animate-float-in pt-10 text-center sm:pt-16">
          ...
          <h1 className="mx-auto mt-6 max-w-3xl text-4xl font-bold leading-[1.05] sm:text-6xl">
            A profile page that feels like a
            <span className="block bg-gradient-to-r from-primary to-foreground bg-clip-text text-transparent">frosted piece of glass</span>
          </h1>
          <p className="mx-auto mt-5 max-w-xl text-base text-muted-foreground">
            Video backgrounds, ambient music, smooth animations and a builder with a real-time phone preview.
          </p>

          <form className="glass mx-auto mt-8 flex max-w-md items-center gap-2 rounded-full p-1.5 pl-4" onSubmit={(e) => e.preventDefault()}>
            <span className="text-sm font-semibold text-muted-foreground">Spider/</span>
            ...
            <Link to="/auth" search={{ mode: "signup", u: username || undefined }} onClick={() => { if (username) { try { sessionStorage.setItem("halo:desired-username", username); } catch {} } }} className="btn-primary shrink-0">
        ...
        <section className="mt-20 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {FEATURES.map((f) => <article key={f.title} className="glass-panel p-6"><f.icon className="h-6 w-6 text-primary" /><h2 className="mt-4 font-display text-base font-semibold">{f.title}</h2><p className="mt-1.5 text-sm text-muted-foreground">{f.body}</p></article>)}
        </section>

        <section className="glass-panel mt-20 flex flex-col items-center gap-4 p-10 text-center">
          <h2 className="max-w-lg text-2xl font-bold sm:text-3xl">Your audience clicks once. Make it count.</h2>
          <p className="max-w-md text-sm text-muted-foreground">Free to start. Upgradeable to Pro for premium badges and extras.</p>
          <Link to="/auth" search={{ mode: "signup" }} className="btn-primary">Build my page</Link>
        </section>
      </main>
