...
export const Route = createFileRoute("/admin-badges/edit")({
  validateSearch: (search: Record<string, unknown>) => ({ badge: typeof search.badge === "string" ? search.badge : "" }),
...
function BadgeEditor() {
  ...
  const { badge } = Route.useSearch();
  const { user, loading } = useAuth();
  const { profile: myProfile } = useMyProfile(user?.id);
  const isAdmin = useIsAdmin(user?.id, user?.email);
  const [profiles, setProfiles] = useState<Profile[]>([]);
  const [owner, setOwner] = useState<Profile | null>(null);
  const [selected, setSelected] = useState<ProfileBadge | null>(() => PROFILE_BADGES.find((item) => item.id === badge) ?? null);
  ...
  useEffect(() => {
    if (isAdmin !== true || !user || !badge) return;
    let cancelled = false;
    const fallback = PROFILE_BADGES.find((item) => item.id === badge) ?? null;
    if (fallback) {
      setSelected(fallback); setName(fallback.name); setDescription(fallback.description || ""); setIcon(fallback.icon || "award"); setEmoji(fallback.emoji || ""); setImageUrl(fallback.imageUrl || ""); setColor(fallback.color || "#ef4444"); setGlowColor(fallback.glowColor || fallback.color || "#ef4444");
    }
    void loadAdminProfiles().then((rows) => {
      if (cancelled) return;
      const ownerData = rows.find((p) => p.id === user.id) ?? myProfile ?? null;
      const found = mergeDefinitions(readDefinitions(ownerData?.social_links)).find((item) => item.id === badge) ?? fallback;
      setOwner(ownerData); setProfiles(rows);
      if (found) { setSelected(found); setName(found.name); setDescription(found.description || ""); setIcon(found.icon || "award"); setEmoji(found.emoji || ""); setImageUrl(found.imageUrl || ""); setColor(found.color || "#ef4444"); setGlowColor(found.glowColor || found.color || "#ef4444"); }
    }).catch(() => {
      if (!cancelled) setOwner(myProfile ?? null);
    }).finally(() => { if (!cancelled) setBusy(false); });
    return () => { cancelled = true; };
  }, [isAdmin, user, badge, myProfile]);

  const assigned = useMemo(() => new Set(profiles.filter((profile) => extractBadges(profile.social_links).some((item) => item.id === badge)).map((profile) => profile.id)), [profiles, badge]);
  ...
  const saveBadge = async () => {
    ...
    const updated: ProfileBadge = { ...selected, name: name.trim() || "Badge", description: description.trim(), icon: icon.trim() || "award", emoji: emoji.trim() || undefined, imageUrl: imageUrl.trim() || undefined, color: cleanColor, glowColor: cleanGlow };
    setSaving(true);
    try {
      const definitions = mergeDefinitions(readDefinitions(owner.social_links)).map((item) => item.id === selected.id ? updated : item);
      await adminMutateProfile(owner.id, { social_links: definitionLinks(owner.social_links, definitions) });
      await Promise.all(profiles.filter((profile) => extractBadges(profile.social_links).some((item) => item.id === selected.id)).map((profile) => adminMutateProfile(profile.id, { social_links: mergeBadges(profile.social_links, extractBadges(profile.social_links).map((item) => item.id === selected.id ? updated : item)) })));
      setSelected(updated); setOwner((current) => current ? { ...current, social_links: definitionLinks(current.social_links, definitions) } : current);
      toast.success("Badge saved and synced to assigned players");
  ...
  const togglePlayer = async (profile: Profile) => {
    if (!selected) return;
    const current = extractBadges(profile.social_links); const has = current.some((item) => item.id === selected.id); const next = has ? current.filter((item) => item.id !== selected.id) : [...current, selected];
    try {
