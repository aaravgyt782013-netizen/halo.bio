...
function AdminBadges() {
  const navigate = useNavigate(); const { user, loading } = useAuth(); const isAdmin = useIsAdmin(user?.id, user?.email); const [badges, setBadges] = useState<ProfileBadge[]>(PROFILE_BADGES); const [busy, setBusy] = useState(true);
  useEffect(() => { if (!loading && !user) navigate({ to: "/auth" }); }, [loading, user, navigate]);
  useEffect(() => { if (!isAdmin || !user) return; void db.from("profiles").select("*").eq("id", user.id).maybeSingle().then(({ data }) => { setBadges(mergeDefinitions(readDefinitions((data as Profile | null)?.social_links))); setBusy(false); }); }, [isAdmin, user]);
