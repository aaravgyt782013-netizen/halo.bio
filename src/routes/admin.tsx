      ...
      <section className="glass-panel p-5 sm:p-6 shadow-lift">
        ...
        <div className="space-y-2.5">
          ...
          {filtered.map((p) => <div key={p.id} className="surface flex flex-wrap items-center gap-3 rounded-xl border border-border/70 p-3.5">
            ...
            <div className="min-w-40 flex-1"><div className="flex flex-wrap items-center gap-2"><span className="text-sm font-semibold">{p.username ? `@${p.username}` : "— no username —"}</span>{p.is_premium && <span className="rounded-full bg-primary/10 px-2 py-0.5 text-[10px] font-bold text-primary">PRO</span>}{p.is_banned && <span className="rounded-full bg-destructive/10 px-2 py-0.5 text-[10px] font-bold text-destructive">BANNED</span>}{p.is_flagged && <span className="rounded-full bg-amber-500/15 px-2 py-0.5 text-[10px] font-bold text-amber-600">FLAGGED</span>}</div><p className="mt-0.5 text-xs text-muted-foreground">{p.display_name ?? "unnamed"} · {p.views || 0} views</p><div className="mt-1 flex flex-wrap items-center gap-3"><Link to="/admin-badges" className="inline-flex items-center gap-1 text-[11px] font-semibold text-red-400 hover:text-red-300"><Award className="h-3 w-3" /> Manage badges</Link><button type="button" onClick={() => openPasswordModal(p)} className="inline-flex items-center gap-1 text-[11px] font-semibold text-primary hover:opacity-80"><KeyRound className="h-3 w-3" /> Change password</button></div></div>
