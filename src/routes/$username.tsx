...
function PublicProfile() {
  ...
  const handleEnter = async () => {
    ...
    } catch (err) { console.warn("Local storage err:", err); }
  ...
  return <div className="relative h-screen w-full">
    <ProfileView profile={profile} links={links} onEnter={handleEnter} onLinkClick={(link) => {
      try {
    ...
    {badges.length > 0 && <div className="fixed inset-x-0 top-1/2 z-[70] flex justify-center translate-y-[105px] px-6">
...
function ProfileNotFound() {
  ...
  useEffect(() => {
    ...
    void fetchProfileByUsername(username).then((res) => {
      if (res?.profile?.username && !res.profile.is_banned) setClientProfile(res);
    });
  }, [username]);
  ...
  if (clientProfile) {
    ...
    return <div className="relative h-screen w-full">
      <ProfileView profile={clientProfile.profile} links={clientProfile.links} onEnter={async () => {
        try {
      ...
      {badges.length > 0 && <div className="fixed inset-x-0 top-1/2 z-[70] flex justify-center translate-y-[105px] px-6"><ProfileBadges badges={badges} /></div>}
    </div>;
  }

  return <div className="relative flex min-h-screen flex-col items-center justify-center gap-4 px-5 text-center">
    ...
    <div className="glass-panel max-w-md p-8 animate-float-in">
      ...
      <div className="mt-6 flex justify-center gap-3"><Link to="/auth" search={{ mode: "signup", u: username }} className="btn-primary">Claim @{username} now</Link><Link to="/" className="btn-ghost">Explore Spider Website</Link></div>
    </div>
  </div>;
}
