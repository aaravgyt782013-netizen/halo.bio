...
import { Award, Check, Plus, Search, Trash2 } from "lucide-react";
...
export function AdminBadgeManager({ initialMemberId = "" }: { initialMemberId?: string }) {
  ...
  const createBadge = async () => {
    ...
    const badge: ProfileBadge = { id: `${idBase || "custom"}-${Date.now().toString(36)}`, name: cleanName, description: description.trim() || "Custom Spider Website badge.", icon: "award", emoji: emoji.trim().slice(0, 4), color, glowColor };
    setSaving(true);
    try { await persistDefinitions([...definitions, badge]); setName(""); setDescription(""); setEmoji("⭐"); toast.success(`${cleanName} badge created`); }
  ...
  return <section className="glass-panel overflow-hidden" id="manage-badges">
    ...
    <div className="grid lg:grid-cols-[250px_1fr]">
      ...
      <div className="space-y-4 p-5">
        <div className="rounded-2xl border border-white/10 bg-black/20 p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[.16em] text-red-400">Create new badge</p><p className="mt-1 text-sm text-white/50">Emoji + icon color + glow color</p></div><div className="grid h-12 w-12 place-items-center rounded-xl border border-white/10 bg-black/30 text-2xl" style={{ color, filter: `drop-shadow(0 0 12px ${glowColor})` }}>{emoji || "⭐"}</div></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="text-xs text-white/55">Name<input className={input} value={name} onChange={(e) => setName(e.target.value)} placeholder="Creator" /></label><label className="text-xs text-white/55">Description<input className={input} value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Special community badge" /></label><label className="text-xs text-white/55">Emoji<input className={input} value={emoji} onChange={(e) => setEmoji(e.target.value)} placeholder="⭐" maxLength={8} /><span className="mt-2 flex flex-wrap gap-1">{emojiPresets.map((x) => <button type="button" key={x} onClick={() => setEmoji(x)} className="rounded-lg border border-white/10 bg-white/5 px-2 py-1 text-base hover:bg-white/10">{x}</button>)}</span></label><div className="grid grid-cols-2 gap-2"><label className="text-xs text-white/55">Icon color<input type="color" className="mt-1 h-10 w-full rounded-xl border border-white/10 bg-black/20 p-1" value={color} onChange={(e) => setColor(e.target.value)} /></label><label className="text-xs text-white/55">Glow color<input type="color" className="mt-1 h-10 w-full rounded-xl border border-white/10 bg-black/20 p-1" value={glowColor} onChange={(e) => setGlowColor(e.target.value)} /></label></div></div><button onClick={() => void createBadge()} disabled={saving} className="mt-4 inline-flex items-center gap-2 rounded-xl bg-red-500 px-4 py-2.5 text-sm font-semibold hover:bg-red-400 disabled:opacity-50"><Plus className="h-4 w-4" />{saving ? "Creating…" : "Create badge"}</button></div>
