...
export function FrostedGlassToggle({ intensity = "medium", currentOpacity, currentBlur, onChange }: Props) {
  const matchedPreset = GLASS_PRESETS.find(
    (p) => p.id === intensity || (Math.abs(p.opacity - currentOpacity) < 0.05 && Math.abs(p.blur - currentBlur) <= 3),
  );
            ...
            return (
              <button key={preset.id} type="button" onClick={() => onChange({ intensity: preset.id, opacity: preset.opacity, blur: preset.blur })} className={`relative flex flex-col items-start p-3 rounded-xl border text-left transition-all active:scale-[0.98] overflow-hidden ${isSelected ? "border-primary bg-primary/10 shadow-sm ring-2 ring-primary/20" : "border-border/80 bg-secondary/40 hover:bg-secondary hover:border-primary/40 text-muted-foreground hover:text-foreground"}`}>
                ...
                <div className="flex items-center gap-1.5 w-full">
                  ...
                  <span className={`text-xs font-bold truncate ${isSelected ? "text-foreground" : "text-foreground/80"}`}>{preset.label}</span>
                </div>
