...
export function BadgesEditor({ profile }: Props) {
  ...
  const allBadges = useMemo(() => {
    ...
    obtained.forEach((badge) => map.set(badge.id, badge));
    return [...map.values()];
  ...
  return <div className="space-y-5 w-full min-w-0">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-border/60 pb-3">
      <div><h2 className="flex items-center gap-2 text-sm font-bold text-foreground"><Award className="h-4 w-4 text-primary" /> Badges</h2><p className="mt-0.5 text-xs text-muted-foreground">All badges with their icons, names and descriptions. Obtained badges are highlighted.</p></div>
    ...
    <div className="grid gap-2 sm:grid-cols-2">{visible.map((badge) => { const owned = obtained.some((b) => b.id === badge.id); return <button key={badge.id} type="button" onClick={() => setSelected(badge)} className={`group flex items-start gap-3 rounded-2xl border p-3.5 text-left transition-all hover:-translate-y-px ${owned ? "border-primary/30 bg-primary/[.06]" : "border-border/70 bg-card/40 hover:border-primary/20"}`}><BadgeGlyph badge={badge} /><span className="min-w-0 flex-1"><span className="block text-sm font-semibold text-foreground">{badge.name}</span><span className="mt-1 block text-[11px] leading-relaxed text-muted-foreground">{badge.description}</span><span className={`mt-2 inline-flex rounded-full px-2 py-0.5 text-[9px] font-semibold ${owned ? "bg-primary/15 text-primary" : "bg-secondary text-muted-foreground"}`}>{owned ? "OBTAINED" : "NOT OBTAINED"}</span></span>{owned && <Check className="h-4 w-4 shrink-0 text-primary" />}</button>; })}</div>
