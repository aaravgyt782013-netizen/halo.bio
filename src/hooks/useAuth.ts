...
export function useAuth() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setSession(current);
      setLoading(false);
      if (current) void auth.getSession().catch(() => null);
    };
    void load();
    return () => {
      cancelled = true;
...
export function useIsAdmin(userId: string | undefined, email?: string | null) {
  ...
  useEffect(() => {
    ...
    void auth.getSession().then(({ data }) => {
      ...
      if (currentEmail === OWNER_EMAIL || data.session?.user?.role === "admin") {
        setIsAdmin(true);
        return;
      }
      void db.from("user_roles").select("role").eq("user_id", userId).eq("role", "admin").maybeSingle()
        .then(({ data: role }) => {
      ...
      console.warn("Could not load authenticated session:", error);
      if (!cancelled) setIsAdmin(false);
    });
